package com.vehicle.common.utils.poi;

import com.vehicle.common.core.text.Convert;
...
import com.vehicle.common.utils.reflect.ReflectUtils;
import com.vehicle.framework.aspectj.lang.annotation.Excel;
import com.vehicle.framework.aspectj.lang.annotation.Excel.ColumnType;
import com.vehicle.framework.aspectj.lang.annotation.Excel.Type;
import com.vehicle.framework.aspectj.lang.annotation.Excels;
...
public class ExcelUtil<T> {
    ...
    private void init(List<T> list, String sheetName, Type type, List<String> excludeColumn) {
        if (list == null) {
            list = new ArrayList<T>();
        }
        this.list = list;
        this.sheetName = sheetName;
        this.type = type;
        createExcelField(excludeColumn);
        createWorkbook();
    }
                ...
                for (Map.Entry<Integer, Field> entry : fieldsMap.entrySet()) {
                    ...
                    if (String.class == fieldType) {
                        if (val instanceof Date) {
                            val = DateUtils.parseDateToStr("yyyy-MM-dd", (Date) val);
                        } else {
    ...
    public AjaxResult exportExcel(List<T> list, String sheetName, List<String> excludeColumn) {
        this.init(list, sheetName, Type.EXPORT, excludeColumn);
        return exportExcel();
    }
    ...
    private String encodingFilename(String filename) {
        ...
        //filename = DateUtils.dateTimeNow() + "_" + filename + ".xlsx";
        return filename;
    }
    ...
    public static String getAbsoluteFile(String filename) {
        String downloadPath = VehicleConfig.getDownloadPath() + filename;
    ...
    private Object getTargetValue(T vo, Field field, Excel excel) throws Exception {
        Object o;
        if (!vo.getClass().getSimpleName().equalsIgnoreCase(field.getDeclaringClass().getSimpleName())) {
            if (field.getDeclaringClass().getSimpleName().equalsIgnoreCase(VhVehicleInfo.class.getSimpleName())) {
                o = field.get(((VhLedgerMain) vo).getVehicleInfo());
            } else {
                o = field.get(vo);
            }
        } else {
...
//        for (Field field : tempFields) {
        for (int i = 0; i < tempFields.size(); i++) {
            ...
            // 判断是否是基本类型
            if (!StringUtils.isJavaClass(field.getType())) {
                tempFields.addAll(Arrays.asList(field.getType().getSuperclass().getDeclaredFields()));
                tempFields.addAll(Arrays.asList(field.getType().getDeclaredFields()));
            }
            // 单注解
    ...
    private void createExcelField(List<String> excludeStr) {
        this.fields = new ArrayList<Object[]>();
        List<Field> tempFields = new ArrayList<>();
        tempFields.addAll(Arrays.asList(clazz.getSuperclass().getDeclaredFields()));
        tempFields.addAll(Arrays.asList(clazz.getDeclaredFields()));
//        for (Field field : tempFields) {
        for (int i = 0; i < tempFields.size(); i++) {
            ...
            //排除不导出列
            if (excludeStr.contains(field.getName())) {
                tempFields.remove(i);
                System.out.println();
                System.out.println("@@@@@@@@@@##########----不导出列 : " + field.getName());
            }
        }
        for (Field field : tempFields) {
            // 单注解
    ...
    public static String exportTxt(String context, String filename) {
        FileWriter fw = null;
        try {
            fw = new FileWriter(getAbsoluteFile(filename));
            fw.write(context);
            fw.flush();
            return filename;
        } catch (Exception e) {
